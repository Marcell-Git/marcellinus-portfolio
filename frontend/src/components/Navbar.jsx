...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-primary-bg/90 backdrop-blur-md py-4 shadow-sm' : 'py-6 bg-transparent'}`}>
      <div className="container flex justify-between items-center">
        ...
        <div className="hidden md:flex gap-10 items-center">
          {navLinks.map((link) => (
            <a key={link.name} href={link.href} className="font-medium text-text-secondary hover:text-accent relative group text-[0.95rem] transition-colors">
              {link.name}
